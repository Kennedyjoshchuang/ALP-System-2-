...
app.post('/api/job-orders/bulk', async (req, res) => {
  try {
    ...
    while (attempts < 5 && !success) {
      insertedJOs = [];
      const { data: existingJOs, error: fetchErr } = await supabase
        .from('job_orders')
        .select('id')
        .like('id', `${year}.%`);
      ...
      let maxSeq = 0;
      if (existingJOs && existingJOs.length > 0) {
        for (const jo of existingJOs) {
          const parts = jo.id.split('.');
          if (parts.length === 3 && parts[0] === String(year)) {
            const seq = parseInt(parts[2], 10);
            if (!isNaN(seq) && seq > maxSeq) {
      ...
      const rowsToInsert = [];
      const localInsertedList = [];

      for (const jo of jobOrders) {
        currentSeq++;
        const seqStr = String(currentSeq).padStart(4, '0');
        const id = `${year}.${month}.${seqStr}`;
        const { quotationId, customerName, jobDescription, phone, email, rate, quantity, quoteValidity, items } = jo;
      ...
      const { error: bulkInsertErr } = await supabase.from('job_orders').insert(rowsToInsert);
      if (bulkInsertErr) {
        if (bulkInsertErr.code === '23505') {
          attempts++;
          await new Promise(resolve => setTimeout(resolve, Math.random() * 50));
          continue;
        }
        insertError = bulkInsertErr;
        break;
      }

      insertedJOs = localInsertedList;
      success = true;
    }

    if (!success) {
      return handleError(res, insertError || new Error('Failed to bulk insert JOs'), 'POST job_orders/bulk');
    }
...
app.get('/api/invoices/next-number', async (req, res) => {
  try {
    const nextId = await getNextInvoiceId(supabase, 'ALP');
...
app.post('/api/invoices', async (req, res) => {
  ...
  try {
    ...
    if (!id || tempIdPattern.test(id)) {
      const originalId = id;
      id = await getNextInvoiceId(supabase, 'ALP');
...
app.put('/api/invoices/*id/settle', async (req, res) => {
  ...
  const paymentProofVal = Array.isArray(paymentProofPhoto) ? JSON.stringify(paymentProofPhoto) : (paymentProofPhoto || null);
  const taxProofVal = Array.isArray(taxDeductionProof) ? JSON.stringify(taxDeductionProof) : (taxDeductionProof || null);
  const settlePaidDate = paidDate || new Date().toISOString();
  ...
  try {
    ...
    const { error: invErr } = await supabase.from('invoices').update({ 
      ...
      tax_deduction_proof: taxProofVal,
      paymentProofPhoto: paymentProofVal,
      paidDate: settlePaidDate
    }).eq('id', id);
...
app.get('/api/employee-accounts', async (req, res) => {
  const { data, error } = await supabase.from('employee_accounts').select('id, username, role');
